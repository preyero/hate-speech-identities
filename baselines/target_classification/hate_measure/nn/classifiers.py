...
from .layers import HateConstructLayer, TargetIdentityLayer, MultiBinaryLayer, UniTargetIdentityLayer
...
class TargetIdentityClassifier(Model):
    ...
    def __init__(
        ...
        pooling='max', mask_pool=True, uni_output=False
    ):
    ...
    @classmethod
    def build_model(cls, transformer='roberta-base', max_length=512, n_dense=64,
                    dropout_rate=0.1, pooling='max', mask_pool=True, uni_output=False):
        """Builds a model using the Functional API."""
        input_ids = Input(shape=(max_length,),
                          dtype=tf.int32,
                          name='input_ids')
