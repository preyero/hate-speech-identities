...
class TargetIdentityLayer(Layer):
    ...
    This layer includes the prediction of age, disability, gender, sexuality, gender and sexual orientation,
    ...
    def __init__(self, name='target_identity_classification', **kwargs):
        ...
        self.g_so = Dense(1, activation='sigmoid', name='gendersexualorientation')
    ...
    def call(self, inputs):
        # Add output binary layers in alphabetical order
        outputs = [
            self.age(inputs),
...
# Identity Layer modified to predict only one target.
class UniTargetIdentityLayer(Layer):
    """Maps input to target identity prediction.

    This layer includes the prediction of a single identity (group or subgroup).
    """
    def __init__(self, name='uni_target_identity_classification', **kwargs):
        super(UniTargetIdentityLayer, self).__init__(name=name, **kwargs)
        self.identity = Dense(1, activation='sigmoid', name='identity')
