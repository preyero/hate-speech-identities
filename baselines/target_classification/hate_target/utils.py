...
def analyze_experiment(path, soft=False, verbose=True, thresholds=None):
    ...
    if len(test_scores.shape) == 1:
        # Reshape test scores in uni-output models: label loss is empty.
        n_folds = test_scores.shape[0]
        test_scores = test_scores.reshape(n_folds, 1)
...
def cv_wrapper(x, y, model_builder, model_kwargs={}, compile_kwargs={},
               ...
               random_state=1, store_models=False, verbose=False,
               cv_verbose=False):
    ...
    history_refit : tf.keras.callbacks.History (optional)
        ...
        refit=True.
    """
    # x, y, random_state, cv_verbose = inputs, y_true, None, True
    # store_models, unwrap_predictions, refit_fold = False, True, True
    # verbose, callbacks, refit = True, [callback], True
    # and all imports
    ...
    # train_idx, test_idx = next(cv.split(x[0]))
    # fold_idx = 1
    for fold_idx, (train_idx, test_idx) in enumerate(cv.split(x[0])):
        ...
        # Compile model and fit model to training data
    ...
    if unwrap_predictions:
        ...
        if n_outputs > 1:
            ...
            test_predictions = [
                np.concatenate([test_predictions[fold]
                                for fold in range(n_folds)], axis=0)[sorted_idxs]]
...
def cv_wrapper_memory_friendly(
    ...
    unwrap_predictions=False, callbacks=None, sample_weights=None,
    random_state=1, cv_verbose=False, save=None):
    """Cross-validate a model and evaluate its predictive accuracy.
...
def memory_friendly_helper(
    ...
    print(gpus)
    # Request specific GPU: Data Card 1: os.environ["CUDA_VISIBLE_DEVICES"]="1"
